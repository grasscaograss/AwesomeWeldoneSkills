...
本仓库是**多上下文单体**：一个耦合代码库，领域拆成几个有界上下文（bounded context），词汇表、知识、ADR 都按上下文轴组织。所有路径相对于项目根目录：

| 路径 | 内容 | 查询方式 |
|------|------|----------|
| `archive/INDEX.md` | 全局索引，含关键词标签 | 关键词匹配、领域浏览入口 |
| `archive/CONTEXT-MAP.md` | 上下文索引 + 共享内核 + 上下文关系 | 先读它定位相关上下文 |
| `archive/contexts/<ctx>/CONTEXT.md` | 各上下文的术语表（每上下文一份） | 术语查询 |
| `archive/records/` | 按日期的会话记录（跨上下文） | 时间范围检索 |
| `archive/contexts/<ctx>/knowledge/<domain>/` | 子领域知识，按上下文嵌套分类 | 上下文→领域浏览 |
| `docs/adr/` | 架构决策记录（系统级；上下文范围的在 `archive/contexts/<ctx>/docs/adr/`） | 关键词匹配 |
| `archive/reviews/` | 月度/年度回顾 | 关键词匹配 |

上下文与子领域一览（详见 `archive/CONTEXT-MAP.md`）：

- **weld-core/**（共享内核 `WeldSeam`）：`weld-seam/` `wsg-merge/` `transition-line/` `weld-template/`
- **robotics/**（共享内核 `Calculator`/坐标）：`coordinate/` `coarse-positioning/`
- **orchestration/**（共享内核 `Executor`/状态）：`workflow/` `scanning/`
- **peripheral/**：`frontend/` `device-robot/` `capacity/` `weld-tracking/` `tools/`

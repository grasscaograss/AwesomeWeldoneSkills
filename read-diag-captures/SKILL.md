...
三个 EventSource：`Robim-WeldPlanning`（焊缝规划）、`Robim-VisionScan`（扫描姿态）、`Robim-Workflow`（FSM 状态机）。完整事件表 + 参数语义 + 典型诊断场景在 catalog 文件里。
...
### references/
- `eventsource-catalog.md` —— 三个 EventSource（`Robim-WeldPlanning`/`Robim-VisionScan`/`Robim-Workflow`）的全部事件表：id、方法名、Level、参数顺序与语义、埋点位置、典型诊断场景。**解读事件流时加载此文档**。本表基于实际 `*EventSource.cs` 源码核对，是事件目录的权威版本。
